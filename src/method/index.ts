import { createRoot, insert, type Node } from '../tree/node.ts';
...
export const insertItem = <T>(router: Router<T>, method: string, path: string, item: T): void => {
  if (method !== '') {
    ...
    router.length === 3 && router.push(createRoot() as any, new Map<string, T>() as any);

    path.includes('*') ? insert(router[3]!, 1, path, 1, item) : router[4]!.set(path, item);
  }
};
